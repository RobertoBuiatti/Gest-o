...
export function Reports() {
	...
	const [editingFixedCostData, setEditingFixedCostData] = useState<{
		name: string;
		amount: number;
		dueDay: number;
		isActive: boolean;
	...
	const [newFixedCostData, setNewFixedCostData] = useState<{
		name: string;
		amount: number;
		dueDay: number;
		isActive: boolean;
...
{
queryKey: ["month-summary", activeSystem, year, month],
queryFn: async () => {
const response = await api.get(
...
queryKey: ["fixed-costs", activeSystem, year, month],
queryFn: async () => {
...
{
queryKey: [isSalon ? "top-services" : "top-products", activeSystem, year, month],
queryFn: async () => {
const startDate = new Date(year, month - 1, 1).toISOString();
...
DailyReport[]
>({
queryKey: ["daily-report", activeSystem, year, month],
queryFn: async () => {
const startDate = new Date(year, month - 1, 1).toISOString();
const endDate = new Date(year, month, 0).toISOString();
const response = await api.get(
					...
					{loadingFixedCosts ? (
						...
						<div className={styles.fixedCostsList}>
							{fixedCosts.map((fc: any) => (
								<div
									key={fc.id}
									className={styles.fixedCostItem}
								>
									{editingFixedCostId === fc.id ? (
										<div className={styles.fixedCostEdit}>
											<input
												className={styles.input}
												value={
													editingFixedCostData.name
												}
												onChange={(e) =>
													setEditingFixedCostData({
														...editingFixedCostData,
														name: e.target.value,
													})
												}
											/>
										...
										<div className={styles.fixedCostRow}>
											<div
												className={styles.fixedCostName}
											>
												{fc.name}
											</div>
											<div
												className={
													styles.fixedCostAmount
												}
											>
